...
SceneLoad::SceneLoad(const SceneLoad& other){}
...
void SceneLoad::init(){
  ...
  for(int i=0;i<3;i++){
    ...
    if(i< total_num_savegame_ ){
      ...
      ((InputField*)getLastObject())->set_text("arial.ttf", "New Game", 90, color_text_button);
      ((InputField*)getLastObject())->changeDefaultText("New Game");
      ((InputField*)getLastObject())->changeMaxInput(15);
    }
  } 

  addObject(new Button("Create"));
  ((Entity*)getLastObject())->set_position(300, 610);
  ((Entity*)getLastObject())->set_scale(1000, 115);
  ((Entity*)getLastObject())->set_color(100, 100, 100);

  ((Button*)getLastObject())->set_text("arial.ttf", "Create the NewGame", 90, color_text_button);
  //-------------------------
        ...
        if ( input_event.mouse_key_input == -1 && s_delay_time > 100 ){
          ...
          if ( std::string(((Button*)(list_obj_[i]))->my_name_).length() > 1 ){
            ...
            if ( ((Button*)(list_obj_[i]))->rect_.x > 800 ){
              ...
              GameManager::Instance()->set_is_load_game(true);
              free(field);
